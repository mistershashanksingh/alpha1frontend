...
const CURRENT_PARTNERS = [
  { name:"Cyberstanc",    category:"Technology Partner",   icon:"🛡️" },
  { name:"AmZetta",       category:"Storage OEM",          icon:"🗄️" },
  { name:"ManageEngine",  category:"Gold Partner",         icon:"⚙️" },
  { name:"Zeperon",       category:"Network Partner",      icon:"⚡" },
  { name:"[Partner 5]",   category:"Reseller Partner",     icon:"🤝" },
  { name:"[Partner 6]",   category:"Channel Partner",      icon:"📡" },
  { name:"[Partner 7]",   category:"SI Partner",           icon:"🔗" },
  { name:"[Partner 8]",   category:"Technology Partner",   icon:"💡" },
];
      ...
      <section style={{ padding:"40px 24px 80px" }}>
        <div className="container">
          <div style={{ textAlign:"center", marginBottom:52 }}>
            <div className="section-badge">🏷️ Partner Types</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:28 }}>
            {CATEGORIES.map((c, i) => (
              <div key={i} className="glass-card" style={{ padding:36 }}>
                <div style={{ width:60, height:60, borderRadius:14, background:`rgba(30,136,229,0.1)`, border:`1px solid ${c.color}33`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:26, marginBottom:20 }}>
                  {c.icon}
                </div>
                <div style={{ fontSize:11, fontWeight:700, color:c.color, letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:8 }}>Partner Type</div>
                <h3 style={{ fontWeight:800, fontSize:20, color:"#fff", marginBottom:14 }}>{c.type}</h3>
                <p style={{ fontSize:14, color:"rgba(255,255,255,0.6)", lineHeight:1.75, marginBottom:20 }}>{c.desc}</p>
                <h4 style={{ fontSize:12, fontWeight:700, color:"rgba(255,255,255,0.4)", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:12 }}>Partner Benefits</h4>
                <ul style={{ listStyle:"none", display:"flex", flexDirection:"column", gap:8 }}>
                  {c.benefits.map((b, j) => (
                    <li key={j} style={{ display:"flex", gap:8, fontSize:13, color:"rgba(255,255,255,0.65)" }}>
                      <span style={{ color:c.color, flexShrink:0 }}>✓</span>{b}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="container">
          <div className="glass-md" style={{ borderRadius:20, padding:"60px 40px", textAlign:"center" }}>
            ...
            <div style={{ display:"flex", gap:16, justifyContent:"center", flexWrap:"wrap" }}>
              ...
              <a href="mailto:partners@alpha1itsolutions.com" className="btn-outline">📧 partners@alpha1itsolutions.com</a>
            </div>
          </div>
        </div>
      </section>
